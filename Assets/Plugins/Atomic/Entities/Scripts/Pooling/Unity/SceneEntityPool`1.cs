...
namespace Atomic.Entities
{
    ...
    public abstract class SceneEntityPool<E, P> : MonoBehaviour, IEntityPool<E>
        where E : IEntity
        where P : SceneEntity, E
    {
#if ODIN_INSPECTOR
        ...
        public async ValueTask InitAsync(int initialCount)
        {
            ...
            P[] entities = await SceneEntity.CreateAsync(_prefab, initialCount, _container);
        ...
        {
            P sceneEntity = SceneEntity.Cast<P>(entity);
        ...
        {
            P entity = SceneEntity.Create(_prefab, _container);
            this.OnCreate(entity);
            return entity;
        }
        ...
        /// </example>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static TPool Create<TPool>(in CreateArgs args) where TPool : SceneEntityPool<E, P>
        {
        ...
        /// </example>
        [MethodImpl(MethodImplOptions.AggressiveInlining)]
        public static void Destroy(SceneEntityPool<E, P> pool, float t = 0)
        {
            if (pool)
            {
                pool.Dispose();
